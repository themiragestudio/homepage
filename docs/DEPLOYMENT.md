...
| 环境 | 地址 | 状态 |
|------|------|------|
| **GitHub Pages（主）** | https://themiragestudio.github.io/homepage/ | ✅ 运行中（推送即部署） |
| 生产服务器 | http://42.192.51.42/ | ⚠️ **已废弃** — 曾运行 Vite Dev 模式（非生产构建），无 SSH 访问权限，请勿使用 |
| 本地开发 | http://localhost:8080 | 手动启动 |

> **说明：** 生产服务器 42.192.51.42 因以下原因正式废弃：
> 1. 检测到响应头中包含 `@react-refresh` 和 `@vite/client`，确认运行的是 Vite Dev 构建而非生产构建
> 2. 当前无 SSH 访问权限，无法修复配置
> 3. GitHub Pages 提供免费、稳定、自动化的部署方案，已完全替代该服务器
>
> **废弃时间：** 2026-03-10 | **废弃原因：** QA 验收发现 P1 问题
